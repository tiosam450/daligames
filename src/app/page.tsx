...
async function getDalyGame(){
  try{
    const res = await fetch('https://sujeitoprogramador.com/next-api/?api=game_day', { cache: 'no-store' })
    return res.json();
  }catch(err){
...
async function getGamesData(){
  try{
    const res = await fetch('https://sujeitoprogramador.com/next-api/?api=games', { cache: 'no-store' })
    return res.json();
  }catch(err){
    console.log("Failed to fetch data")
  }
